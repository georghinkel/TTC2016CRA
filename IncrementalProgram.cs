...
namespace ClassDiagramOptimization
{
    class IncrementalProgram
    {
        static void Main(string[] args)
        {
            ...
            var mai = new ObservingFunc<IClass, IClass, double>((cl_i, cl_j) =>
                cl_i.Encapsulates.OfType<Method>()
                    .SelectMany(m => m.DataDependency)
                    .Intersect(cl_j.Encapsulates)
                    .Count());

            var mmi = new ObservingFunc<IClass, IClass, double>((cl_i, cl_j) =>
                cl_i.Encapsulates.OfType<Method>()
            ...
            var possibleMerges = from cl_i in classModel.Classes
                                 ...
                                 select new
                                 {
                                     ...
                                     MAI_i = mai.Evaluate(cl_i, cl_i),
                                     MAI_j = mai.Evaluate(cl_j, cl_j),
                                     MAI_ij = mai.Evaluate(cl_i, cl_j),
                                     MAI_ji = mai.Evaluate(cl_j, cl_i),
                                     MMI_i = mmi.Evaluate(cl_i, cl_i),
                                     MMI_j = mmi.Evaluate(cl_j, cl_j),
                                     MMI_ij = mmi.Evaluate(cl_i, cl_j),
                                     MMI_ji = mmi.Evaluate(cl_j, cl_i)
                                 };
            ...
            var prioritizedMergesInc = prioritizedMerges.AsNotifiable();
            var nextMerge = prioritizedMergesInc.FirstOrDefault();
            var classCounter = 1;
            while (nextMerge != null && nextMerge.Effect > 0)
            {
                ...
                nextMerge = prioritizedMergesInc.FirstOrDefault();
            }
            stopwatch.Stop();
